...
def genre_series(request, genre_id):
    ...
    return render(request, 'genre_series.html', {'series': series})
...
def fetch_fantasy_anime():
    return fetch_anime_by_genre(10)

def fetch_mecha_anime():
    return fetch_anime_by_genre(18)

def fetch_psychological_anime():
    return fetch_anime_by_genre(40)

def fetch_supernatural_anime():
    return fetch_anime_by_genre(37)

def fetch_slice_of_life_anime():
    return fetch_anime_by_genre(36)

def fetch_thriller_anime():
    return fetch_anime_by_genre(41)

def fetch_sports_anime():
...
def home_anime(request, genre_id):
    try:
        url = f'https://api.jikan.moe/v4/anime?genres={genre_id}&page=1&order_by=year&sort=desc'
        response = requests.get(url)
        response.raise_for_status()
    ...
    # Fetching other genres
    action_animes = fetch_action_anime()
    scifi_animes = fetch_scifi_anime()
    horror_animes = fetch_horror_anime()
    drama_animes = fetch_drama_anime()
    comedy_animes = fetch_comedy_anime()
    ...
    supernatural_animes = fetch_supernatural_anime()
    ...
    posters.extend([anime['images']['jpg']['image_url'] for anime in action_animes])
    posters.extend([anime['images']['jpg']['image_url'] for anime in scifi_animes])
    posters.extend([anime['images']['jpg']['image_url'] for anime in horror_animes])
    posters.extend([anime['images']['jpg']['image_url'] for anime in drama_animes])
    posters.extend([anime['images']['jpg']['image_url'] for anime in comedy_animes])
    ...
    posters.extend([anime['images']['jpg']['image_url'] for anime in supernatural_animes])
    posters.extend([anime['images']['jpg']['image_url'] for anime in slice_of_life_animes])
...
'''
    context = {
        ...
        'history_anime': history_animes,
        'scifi_anime': scifi_animes,
        'drama_anime': drama_animes,
        'comedy_anime': comedy_animes,
        ...
        'supernatural_anime': supernatural_animes,
